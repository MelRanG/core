...
- @Bean으로 생성 된 부분도 new OrderRepository()처럼 새로운 객체를 생성하는데 싱글턴을 보장할 수 있는 이유는 뭘까? 스프링에서는 @Bean으로 스프링 컨테이너에 빈을 등록하고 config에 @Configuration이 있다면 해당 config를 configCGlib라는 자식클래스를 새로 만들어서 이 클래스로 빈을 등록한다. cglib는 바이트코드를 조작하며, 빈을 생성할 경우 스프링컨테이너에 생성된 빈이 있는지 확인하고 있다면 해당 빈을 리턴하고 아니라면 새로운 빈을 생성한다. 이 방법으로 new라고 생성해도 싱글턴을 보장한다.

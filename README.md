...
- 객체가 생성되고 의존성 주입이 발생할 때는 RequestScope로 선언된 빈은 주입되지 않는다. Provider를 사용해서 RequestScope로 선언된 빈을 받아서 생성하면 의존성 주입 시점에 Provider에 DL로 의존성이 주입되고, 해당 빈을 사용할 때 실제로 빈을 생성해서 문제를 해결한다.
